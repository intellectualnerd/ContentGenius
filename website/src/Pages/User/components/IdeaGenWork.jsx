...
const {userInfoData,userContentInfoData}=useUser()
    ...
    const fetchNewIdeas = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.post('http://localhost:4000/getideas',{links:userContentInfoData.webUrlArr}); // Replace with your API endpoint
            ...
            setIdeas(prevIdeas => {
                ...
                const filteredNewIdeas = newIdeas.filter(newIdea => 
                    newIdea && newIdea.title && !prevIdeas.some(prevIdea => prevIdea.title === newIdea.title)
                );
